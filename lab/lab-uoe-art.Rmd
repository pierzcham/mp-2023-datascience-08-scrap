---
title: "Lab - University of Edinburgh Art Collection"
output: 
  tufte::tufte_html:
    css: ../lab.css
...
```{marginfigure}
See the sidebar [here](https://collections.ed.ac.uk/art) and note that there are 3018 pieces in the art collection we're collecting data on. Note that more pieces may have been added or some pieces may have been removed between when this lab was written and when you're working on it.
```
...
-   Download and install the SelectorGadget for your browser.
    ...
    -   See [here](https://selectorgadget.com/) for Chrome (recommended)
...
-   `.Rmd`: R Markdown documents are plain text files containing code snippets and structured text.
...
We'll use the **tidyverse** package for much of the data wrangling and visualisation, the **polite** package to check if we're allowed to scrape the data, the **rvest** package for data scraping. 
...
```{r i.polite, warning=FALSE}
bow("https://collections.ed.ac.uk/art)") |> 
  is.polite()
```
...
```{marginfigure}
**Reminder:** The collection has 3018 pieces in total.
```

That means we give develop a list of URLs (of pages that each have 10 art pieces), and write some code that applies the `scrape_page()` function to each page, and combines the resulting data frames from each page into a single data frame with 3018 rows and 3 columns.
...
...
[sometext]offset=3010  # Pieces 3011-3018
```

We can construct these URLs in R by pasting together two pieces: (1) a common (`root`) text for the beginning of the URL, and (2) numbers starting at 0, increasing by 10, all the way up to 3010.
...
::: box
Work in `lab.Rmd` for the rest of the lab.
:::
